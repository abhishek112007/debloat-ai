...
const DevicePanel: React.FC<DevicePanelProps> = ({ onRefresh }) => {
  ...
  const handleRefresh = async () => {
    ...
    // Show refreshing state for at least 1 second for visual feedback
    setTimeout(() => setIsRefreshing(false), 1500);
  };

  // Row style — no border, just a subtle bg that blends into the sidebar card
  const cardStyle = {
  ...
  return (
    ...
    >
          ...
          >
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--theme-text-secondary)' }}>
                  Device
                </span>
              </div>
              <div className="ml-9">
                {/* Friendly name: manufacturer + model */}
                <div className="text-sm font-semibold leading-snug truncate" style={{ color: 'var(--theme-text-primary)' }}>
                  {device.manufacturer && device.manufacturer !== 'Unknown'
                    ? `${device.manufacturer} ${device.model && device.model !== 'Unknown' ? device.model : ''}`
                    : device.model && device.model !== 'Unknown'
                      ? device.model
                      : device.name}
                </div>
                {/* Serial number - secondary */}
                <div className="text-xs mt-0.5 truncate font-mono opacity-60" style={{ color: 'var(--theme-text-secondary)' }}>
                ...
                <div className="ml-9">
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ background: isLightMode ? 'rgba(0,0,0,0.1)' : 'rgba(255,255,255,0.1)' }}>
                      <motion.div
                        className={`h-full rounded-full ${device.batteryPercentage > 50 ? 'bg-emerald-500' :
      ...
      <motion.button
        ...
        className="mt-6 w-full text-sm flex items-center justify-center gap-2 group disabled:hover:translate-y-0 overflow-hidden relative"
        whileHover={{
          scale: 1.02,
          y: -2,
      ...
      >
        ...
        <motion.div
          ...
          animate={(loading || isRefreshing) ? { rotate: 360 } : { rotate: 0 }}
          transition={(loading || isRefreshing) ? { duration: 1, repeat: Infinity, ease: 'linear' } : { duration: 0.3 }}
          whileHover={!(loading || isRefreshing) ? { rotate: 180, scale: 1.1 } : {}}
        >
          <FiRefreshCw className="w-4 h-4" />
        </motion.div>
        <span className="relative z-10 transition-all duration-200">
          {loading || isRefreshing ? 'Refreshing...' : 'Refresh Packages'}
        </span>
      </motion.button>
    </motion.div>
  );
